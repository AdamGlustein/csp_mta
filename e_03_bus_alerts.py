...
from mta_util import MTA_FEED_UPDATE_TIME, ALERT_ENDPOINTS
...
def pretty_print_alerts(alerts: csp.ts[object]) -> csp.ts[str]:
    ...
    for entity in alerts['entity']:
        ...
        all_alerts.append( f'-- {route_id}: {entity["alert"]["header_text"]["translation"][0]["text"]}' )
...
if __name__ == '__main__':
    csp.run(get_alerts, ALERT_ENDPOINTS['bus'], starttime=datetime.utcnow(), endtime=timedelta(seconds=10), realtime=True)