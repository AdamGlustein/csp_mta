...
from GTFSInputAdapter import GTFSRealtimeInputAdapter, GTFS_DIRECTION, STOP_INFO_DF, LINE_TO_ENDPOINT
from compiled_protobuf import nyct_subway_pb2
...
def get_stop_time_at_station(entity, stop_id):
    ...
    if entity.HasField('trip_update'):
        ...
        for update in stop_time_updates:
            if stop_id in update.stop_id and datetime.fromtimestamp(update.arrival.time) >= datetime.now() : # could be N or S
                return update.arrival.time
    return None

@csp.node
def filter_trains_headed_for_stop(gtfs_msgs: csp.ts[object], stop_id: str) -> csp.ts[object]:
    '''
...
@csp.node
def next_N_trains_at_stop(gtfs_msgs: csp.ts[object], stop_id: str, N: int) -> csp.ts[object]:
    '''
...
def entities_to_departure_board_str(entities, stop_id):
    ...
    for entity in entities:
        ...
        direction = GTFS_DIRECTION[
            entity.trip_update.trip.Extensions[nyct_subway_pb2.nyct_trip_descriptor].direction
        ]
...
def departure_board(platforms: List[Tuple[str, str]], N: int):
    ...
    for service in platforms:
        ...
        dep_str = csp.apply(
            next_N_trains,       
            lambda x, key_=stop_id: entities_to_departure_board_str(x, key_), str
        )
        csp.print('Departure Board', dep_str)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Print a realtime departure board at a set of stations.")
    parser.add_argument('platforms', nargs='+', help='stop_id:service pair: see stops.txt for stop_id')
    parser.add_argument('--show_graph', action='store_true', default=False, help='Boolean flag indicating if the graph should be shown')
    parser.add_argument('--run_graph', action='store_true', default=True, help='Boolean flag indicating if the graph should be run')
    parser.add_argument('--num_trains', type=int, default=5, help='Number of trains for each line to show on the departure board')
    ...
    for platform in args.platforms:
        ...
        if stop_id not in STOP_INFO_DF.index:
            raise ValueError(f'Did not recognize stop_id {stop_id}: see stops.txt for valid stop_ids')
        
        platforms_to_subscribe_to.append((stop_id, train_line))

    if show_graph:
        csp.show_graph(departure_board, platforms_to_subscribe_to, num_trains, graph_filename='departure_board.png')
    if run_graph:
        csp.run(departure_board, platforms_to_subscribe_to, num_trains, starttime=datetime.utcnow(), endtime=timedelta(minutes=1), realtime=True)
